...
import { Volume2, Eraser, Eye, EyeOff, ArrowRight, ArrowLeft, RefreshCw, Check } from 'lucide-react';
...
export default function WritingCanvas({ scriptMode }) {
  ...
  useEffect(() => {
    ...
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
    ctx.strokeStyle = isDarkMode ? '#36d399' : '#38656f'; // Glowing Mint Teal in Dark Mode
    ctx.lineWidth = 10;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-zen-text-muted dark:text-zen-dark-text-muted">
            Trace the character strokes with your finger or mouse cursor.
          </p>
        </div>
        ...
        >
          <Volume2 className="w-4 h-4" /> Listen ({currentItem.romaji})
        </button>
      </div>
          ...
          <div className="w-full flex items-center justify-between mb-4 text-xs font-semibold text-zen-text-muted dark:text-zen-dark-text-muted">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-zen-border/40 dark:border-zen-dark-border hover:bg-zen-surface-container dark:hover:bg-zen-dark-surface-high transition-colors text-xs"
              >
                {showGuide ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
                {showGuide ? 'Hide Outline' : 'Show Outline'}
              </button>

              <button
                onClick={clearCanvas}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-zen-border/40 dark:border-zen-dark-border hover:bg-zen-secondary/10 dark:hover:bg-zen-dark-secondary/20 hover:text-zen-secondary dark:hover:text-zen-dark-primary transition-colors text-xs"
              >
          ...
          <div className="mt-4 w-full flex items-center justify-between text-xs font-semibold text-zen-text-muted dark:text-zen-dark-text-muted">
            {hasDrawn ? (
              <span className="text-zen-primary dark:text-zen-dark-primary flex items-center gap-1">
                <Check className="w-4 h-4" /> Stroke recorded!
              </span>
            ) : (
              <span>Draw strokes with mouse or finger</span>
            )}

            <button
              onClick={handleNext}
              className="px-5 py-2.5 rounded-xl bg-zen-primary dark:bg-zen-dark-primary text-white dark:text-zen-dark-on-primary font-bold shadow-zen-sm flex items-center gap-1.5"
            >
              Salva / Avanti <ArrowRight className="w-4 h-4" />
            </button>
          </div>
        </div>
      </div>
